...
public class RollersIOSim implements RollersIO {
  ...
  private PIDController rollerController = new PIDController(RollerConstants.simkP, 0, RollerConstants.simkD);
  private double desiredVelo = 0.0;
  public RollersIOSim() {
      rollersSim =
      new DCMotorSim(
          LinearSystemId.createDCMotorSystem(
            ROLLERS_GEARBOX, RollerConstants.kRollerInertia, RollerConstants.kRollerMotorGearRatio),
            ROLLERS_GEARBOX); 
  }  
  @Override 
  ...
  public void runVelocity(double velocity){
    ...
    rollersSim.setInputVoltage(rollerController.calculate(rollersSim.getAngularVelocityRPM()/60.0, velocity));
  }
  @Override
  public void stop(){
    runVelocity(0);
